...
const gameTime = 60 * 1000; // 60 seconds
let timer = null;
let gameStart = null;
...
function newGame() {
  ...
  infoElement.textContent = gameTime / 1000;
  timer = null;
  gameStart = null;
  removeClass(gameElement, 'over');
  addClass(gameElement, 'started');
  updateCursorPosition();
}

function getWpm() {
  const correctLetters = document.querySelectorAll('.letter.correct').length;
  const totalWords = correctLetters / 5; // Assuming 5 characters per word on average
  return Math.round((totalWords / gameTime) * 60000);
}
...
function gameOver() {
  ...
  const result = getWpm();
  infoElement.textContent = `WPM: ${result}`;
}
...
function updateCursorPosition() {
  ...
  if (nextLetter || nextWord) {
    cursorElement.style.top = (nextLetter || nextWord).getBoundingClientRect().top + 2 + 'px';
    cursorElement.style.left = (nextLetter || nextWord).getBoundingClientRect()[nextLetter ? 'left' : 'right'] + 'px';
  }
}
...
gameElement.addEventListener('keyup', (ev) => {
  ...
  if (!timer && isLetter) {
    timer = setInterval(() => {
      ...
      const timeLeft = Math.round((gameTime - (Date.now() - gameStart)) / 1000);
      if (timeLeft <= 0) {
        gameOver();
        return;
      }
      infoElement.textContent = timeLeft;
    }, 1000);
  }
